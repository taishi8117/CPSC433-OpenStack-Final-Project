...
public class APIHandler {
	 ...
	 * @throws Exception - when there is no associated network from given IDs or port is already used
	 */
	public int createNewPort(long tenantID, long networkID, int number) throws  Exception{
		...
		Port port = controller.registerNewPort(number);
		if (port == null) {
			throw new Exception("createNewPort(): Port already used");
		}
	...
	public int createNewPort(long tenantID, long networkID) throws Exception{
		...
		Port port = controller.registerNewPort(0);
		if (port == null) {
			//shouldn't happen unless all the ports are used
			throw new Exception("createNewPort(): Port already used");
		}
		return port.num;
	}
